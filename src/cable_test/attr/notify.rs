...
use crate::{
    cable_test::attr::nest::EthtoolCableTestNestAttr, EthtoolAttr,
    EthtoolHeader,
};
...
pub enum EthtoolCableTestNotifyAttr {
    Header(Vec<EthtoolHeader>),
    Status(EthtoolCableTestStatus),
    Nest(Vec<EthtoolCableTestNestAttr>),
    Other(DefaultNla),
}
...
impl From<EthtoolCableTestStatus> for u8 {
    fn from(value: EthtoolCableTestStatus) -> Self {
        match value {
...
impl Nla for EthtoolCableTestNotifyAttr {
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>>
    for EthtoolCableTestNotifyAttr
{
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        match buf.kind() {
                ...
                let nlas = NlasIterator::new(buf.value())
                    .map(|nla| {
                        EthtoolHeader::parse(&nla?).context(
                            "failed to parse ETHTOOL_A_CABLE_TEST_NTF_HEADER",
                        )
                    })
            ...
            ETHTOOL_A_CABLE_TEST_NTF_NEST => {
                let nlas = NlasIterator::new(buf.value())
                    .map(|nla| {
                        EthtoolCableTestNestAttr::parse(&nla?).context(
                            "failed to parse ETHTOOL_A_CABLE_TEST_NTF_NEST",
                        )
                    })
                    .collect::<Result<Vec<_>, _>>()?;
                Ok(Self::Nest(nlas))
            ...
            _ => {
                let other = DefaultNla::parse(buf).context(
                    "failed to parse unknown NLA for cable test notify",
                )?;
                Ok(Self::Other(other))
            }
        }
...
pub(crate) fn parse_cable_test_notify_nlas(
    buffer: &[u8],
    ...
    NlasIterator::new(buffer)
        .map(|nla| {
            ...
            let parsed = EthtoolCableTestNotifyAttr::parse(&nla)
                .context("failed to parse ethtool cable test notify NLA")?;
            Ok(EthtoolAttr::CableTestNotify(parsed))
        })
        .collect()
}
